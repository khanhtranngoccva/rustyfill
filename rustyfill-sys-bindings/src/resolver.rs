...
// The parsed-use-statement vocabulary now lives in `syntaxes`; re-exported here
// so existing `crate::resolver::{...}` import paths keep working while callers
// migrate to the canonical location.
pub use crate::syntaxes::{PathSegment, PathSegmentList, UseKind, UseStatement, Visibility};

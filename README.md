...
- **Atomic-or-resume semantics.** Some mutating operations reserve capacity before doing logical work, so a failure short-circuits with nothing half-done, while other operations perform rollback. Iterator-driven operations (`try_extend`) can't be rolled back because iterators consume their source, and some map/set operations cannot be rolled back because values may have been overwritten; instead they hand back a `Resumable` handle carrying the stranded element plus the unconsumed remainder, letting the caller retry from exactly where it stopped.
